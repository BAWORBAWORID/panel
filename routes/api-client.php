...
use Pterodactyl\Http\Controllers\Api\Client\MusicController;
...
Route::get('/alerts/active', Client\ActiveAlertController::class);
...
Route::prefix('/account')->middleware(AccountSubject::class)->group(function () {
    ...
    // --- FITUR BACKGROUND CUSTOMIZER ---
    Route::get('/background', [Client\BackgroundController::class, 'show']);
    Route::put('/background', [Client\BackgroundController::class, 'update']);
});

Route::prefix('account/music')->group(function () {
    // Ambil semua track + preferences
    Route::get('/',              [MusicController::class, 'index']);

    // Tambah track via YouTube URL
    Route::post('/youtube',      [MusicController::class, 'addYoutube']);

    // Upload file mp3/wav/ogg langsung
    Route::post('/upload',       [MusicController::class, 'upload']);

    // Hapus track tertentu
    Route::delete('/{track}',    [MusicController::class, 'destroy'])
         ->where('track', '[0-9]+');

    // Simpan preferensi player (volume, shuffle, repeat, current_track_id)
    Route::put('/preferences',   [MusicController::class, 'updatePreferences']);
});
...
Route::group([
    ...
    Route::group(['prefix' => '/settings'], function () {
        ...
        Route::put('/visibility', [Client\Servers\SettingsController::class, 'toggleVisibility']);
    });
});
